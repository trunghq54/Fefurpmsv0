...
export default function ProposalWorkspace({
  proposalId, onBack, onChanged,
}: { proposalId: string; onBack: () => void; onChanged?: () => void }) {
  ...
  const load = () => proposalService.getById(proposalId).then((r) => { if (r.success && r.data) setP(r.data); setLoading(false) })
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (res.success) { setSubmitMsg('OK'); onChanged?.(); load() }
...
function InfoSection({ proposal, onSaved }: { proposal: ProposalDto; onSaved: () => void }) {
  const [f, setF] = useState({
    titleVI: proposal.titleVI || '', titleEN: proposal.titleEN || '', trackId: proposal.trackId || '',
    researchType: proposal.researchTypeId ?? (proposal.researchType === 'Applied' ? 1 : 2),
    durationMonths: proposal.durationMonths || 12,
    objectives: proposal.objectives || '', methodology: proposal.methodology || '', expectedOutput: proposal.expectedOutput || '',
    abstractEN: proposal.abstractEN || '', urgency: proposal.urgency || '', novelty: proposal.novelty || '',
    applicationPotential: proposal.applicationPotential || '', transferPotential: proposal.transferPotential || '',
    facilities: proposal.facilities || '', fundingMethod: proposal.fundingMethod || 'PARTIAL',
  })
  const [tracks, setTracks] = useState<TrackDto[]>([])
  const [saving, setSaving] = useState(false); const [saved, setSaved] = useState(false); const [err, setErr] = useState('')
  useEffect(() => { cycleService.getTracks().then((r) => { if (r.success && r.data) setTracks(r.data.filter((t) => t.isActive)) }) }, [])
  ...
  const save = async () => {
    ...
    try {
      const payload: CreateProposalRequest = {
        trackId: f.trackId, titleVI: f.titleVI, titleEN: f.titleEN, researchType: f.researchType,
        durationMonths: f.durationMonths, objectives: f.objectives, methodology: f.methodology, expectedOutput: f.expectedOutput,
        abstractEN: f.abstractEN, urgency: f.urgency, novelty: f.novelty, applicationPotential: f.applicationPotential,
        transferPotential: f.transferPotential, facilities: f.facilities, fundingMethod: f.fundingMethod,
        members: [], budgetItems: [], // rỗng → BE giữ nguyên members/budget (chỉ lưu thông tin lõi)
      }
    ...
    } catch (e: any) { setErr(e?.response?.data?.message || 'Có lỗi xảy ra') }
    ...
    <Card className="p-6 space-y-4">
      ...
      <div className="pt-2 border-t border-gray-100">
        ...
        <div className="space-y-4">
          ...
          <Field label="Khả năng ứng dụng" value={f.applicationPotential} onChange={(v) => set('applicationPotential', v)} />
          <Field label="Khả năng chuyển giao" value={f.transferPotential} onChange={(v) => set('transferPotential', v)} />
...
function MembersSection({ proposalId, canEdit }: { proposalId: string; canEdit: boolean }) {
  ...
  const [loading, setLoading] = useState(true)
  const [n, setN] = useState({ academicTitle: '', fullName: '', memberRoleCode: 'TVC', isSecretary: false, unitName: '', workContent: '', workMonths: 0 })
  const [busy, setBusy] = useState(false)

  const load = () => teamMemberService.getTeamMembers(proposalId).then((r) => { if (r.success && r.data) setMembers(r.data); setLoading(false) })
  useEffect(() => { load() }, [proposalId])
  ...
  const add = async () => {
    ...
    try {
      await teamMemberService.addTeamMember(proposalId, {
        fullName: n.fullName, academicTitle: n.academicTitle || undefined, unitName: n.unitName || undefined,
        workContent: n.workContent || n.fullName, workMonths: n.workMonths, isPi: false,
        isSecretary: n.isSecretary, memberRoleCode: n.memberRoleCode,
      })
      setN({ academicTitle: '', fullName: '', memberRoleCode: 'TVC', isSecretary: false, unitName: '', workContent: '', workMonths: 0 })
      load()
        ...
        {members.map((m) => (
          <div key={m.id} className="flex items-center justify-between px-4 py-3 text-sm">
            <div className="text-gray-800">
              ...
              <span className="text-gray-400"> · {m.isSecretary ? 'Thư ký' : (ROLE_CODES.find((r) => r.code === m.memberRoleCode)?.label || m.memberRoleCode || 'Thành viên')}</span>
