...
function ContentsSection({ proposalId }: { proposalId: string }) {
  ...
  const load = () =>
    proposalContentsService.getContents(proposalId).then((r) => {
  ...
  const addContent = async () => {
    ...
    try {
      await proposalContentsService.createContent(proposalId, {
        contentNumber: contents.length + 1,
        title,
        description: desc,
  ...
  const delContent = async (id: number) => {
    await proposalContentsService.deleteContent(proposalId, id)
    load()
  }
  ...
  return (
    <div className="space-y-4">
      <Card className="p-4 space-y-3">
        ...
        <Button size="sm" onClick={addContent} disabled={busy || !title.trim()}>
          <Plus className="w-4 h-4" /> Thêm
        </Button>
      </Card>
      {contents.length === 0 ? (
        <Empty text="Chưa có nội dung nghiên cứu." />
      ) : (
        contents.map((c) => (
          <ContentCard
            key={c.id}
...
function ContentCard({
  ...
  const addActivity = async () => {
    ...
    try {
      await proposalContentsService.createActivity(proposalId, content.id, {
        ...
        endMonth: em,
        estimatedCost: 0,
        sequence: (content.activities?.length || 0) + 1,
  ...
  const delAct = async (id: number) => {
    await proposalContentsService.deleteActivity(proposalId, id)
    onChanged()
  }
    ...
    <Card className="p-4">
      <div className="flex items-start justify-between">
        <div>
          <p className="font-semibold text-gray-800">
            Nội dung {content.contentNumber}: {content.title}
          </p>
          {content.description && <p className="text-sm text-gray-500 mt-0.5">{content.description}</p>}
        </div>
        <button onClick={onDelete} className="p-1.5 text-red-600 hover:bg-red-50 rounded-lg">
          ...
          <div key={a.id} className="flex items-center justify-between bg-gray-50 rounded-lg px-3 py-1.5 text-sm">
            <span className="text-gray-700">
              ...
              <span className="text-gray-400">
                · tháng {a.startMonth}–{a.endMonth}
              </span>
            </span>
            <button onClick={() => delAct(a.id)} className="p-1 text-red-500 hover:bg-red-50 rounded">
      ...
      <div className="mt-3 grid md:grid-cols-12 gap-2 items-end">
        <div className="md:col-span-4">
          <label className={cellLabel}>Hoạt động</label>
          <Input value={name} onChange={(e) => setName(e.target.value)} />
        </div>
        <div className="md:col-span-4">
          <label className={cellLabel}>Kết quả cần đạt</label>
          <Input value={result} onChange={(e) => setResult(e.target.value)} />
        </div>
        <div className="md:col-span-1">
          <label className={cellLabel}>Từ (th)</label>
          <Input type="number" min={1} value={sm} onChange={(e) => setSm(Number(e.target.value))} />
        </div>
        <div className="md:col-span-1">
          <label className={cellLabel}>Đến (th)</label>
          <Input type="number" min={1} value={em} onChange={(e) => setEm(Number(e.target.value))} />
        </div>
        <div className="md:col-span-2">
          <Button variant="success" onClick={addActivity} disabled={busy || !name.trim()}>
            <Plus className="w-4 h-4" /> Hoạt động
          </Button>
        </div>
      </div>
    </Card>
...
function ProductsSection({ proposalId }: { proposalId: string }) {
  ...
  const load = () =>
    proposalContentsService.getExpectedProducts(proposalId).then((r) => {
  ...
  const add = async () => {
    ...
    try {
      await proposalContentsService.createExpectedProduct(proposalId, {
        productName: name,
        scientificRequirements: req,
        categoryId: catId === '' ? undefined : catId,
  ...
  const del = async (id: number) => {
    await proposalContentsService.deleteExpectedProduct(proposalId, id)
    load()
  }
  ...
  return (
    <div className="space-y-4">
      <Card className="p-4 grid md:grid-cols-12 gap-2 items-end">
        <div className="md:col-span-4">
          <label className={cellLabel}>Tên sản phẩm</label>
          <Input value={name} onChange={(e) => setName(e.target.value)} />
        </div>
        <div className="md:col-span-4">
          <label className={cellLabel}>Tiêu chí/Yêu cầu KH</label>
          <Input value={req} onChange={(e) => setReq(e.target.value)} />
        </div>
        <div className="md:col-span-2">
          <label className={cellLabel}>Dạng SP</label>
      ...
      {products.length === 0 ? (
        <Empty text="Chưa có sản phẩm dự kiến." />
      ) : (
          ...
          {products.map((p) => (
            <div key={p.id} className="flex items-center justify-between px-4 py-3 text-sm">
              <div>
                ...
                {p.scientificRequirements && <span className="text-gray-400"> · {p.scientificRequirements}</span>}
              </div>
              <button onClick={() => del(p.id)} className="p-1.5 text-red-600 hover:bg-red-50 rounded-lg">
                <Trash2 className="w-4 h-4" />
              </button>
            </div>
          ))}
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="text-left text-gray-500 border-b">
              ...
              <th className="px-2">Tổng (₫)</th>
              <th className="px-2">Khoán</th>
              <th className="px-2">Ngoài khoán</th>
              <th className="px-2">NSNN</th>
              <th className="px-2">Khác</th>
              <th></th>
            </tr>
          </thead>
          <tbody>
            {items.map((it, idx) => (
              <tr key={idx} className="border-b last:border-0">
                <td className="py-1.5 pr-2 text-gray-800">
                  {it.categoryName || it.categoryCode || `#${it.categoryId}`}
                </td>
                <td className="px-1">
                  <NumCell value={it.amount} onChange={(v) => setField(idx, 'amount', v)} />
                </td>
                <td className="px-1">
                  <NumCell value={it.sourceKhoan} onChange={(v) => setField(idx, 'sourceKhoan', v)} />
                </td>
                <td className="px-1">
                  <NumCell value={it.sourceNgoaiKhoan} onChange={(v) => setField(idx, 'sourceNgoaiKhoan', v)} />
                </td>
                <td className="px-1">
                  <NumCell value={it.sourceNsnn} onChange={(v) => setField(idx, 'sourceNsnn', v)} />
                </td>
                <td className="px-1">
                  <NumCell value={it.sourceOther} onChange={(v) => setField(idx, 'sourceOther', v)} />
                </td>
                <td>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          <span className="text-sm text-gray-600">
            Tổng: <b className="text-gray-900">{fmt(total)} ₫</b>
          </span>
          <Button variant="success" onClick={save} disabled={saving}>
            {saved ? <CheckCircle className="w-4 h-4" /> : <Save className="w-4 h-4" />}{' '}
            {saved ? 'Đã lưu' : saving ? 'Đang lưu...' : 'Lưu kinh phí'}
          </Button>
        </div>
      </div>
    </div>
...
function LaborSection({ proposalId }: { proposalId: string }) {
  ...
  const load = () =>
    proposalBudgetService.getLaborDetails(proposalId).then((r) => {
  ...
  const saveRow = async (row: LaborDetailResponse) => {
    ...
    try {
      const res = await proposalBudgetService.updateLaborDetail(proposalId, row.id, {
        workDays: row.workDays ?? undefined,
        coefficient: row.coefficient ?? undefined,
        totalResearchHours: row.totalResearchHours,
  ...
  if (rows.length === 0)
    return (
      <Empty text="Chưa có dòng tiền công. Thêm thành viên cho đề tài trước (hệ thống tự tạo dòng tiền công theo thành viên)." />
    )
  return (
    <Card className="p-4 overflow-x-auto">
      <table className="w-full text-sm">
        <thead>
          <tr className="text-left text-gray-500 border-b">
            <th className="py-2 pr-2">Thành viên</th>
            <th className="px-2">Số ngày công</th>
            <th className="px-2">Hệ số</th>
            <th className="px-2">Đơn giá ngày</th>
            <th className="px-2">Tổng tiền</th>
            <th></th>
          </tr>
        </thead>
        <tbody>
          {rows.map((row, idx) => (
            <tr key={row.id} className="border-b last:border-0">
              ...
              <td className="px-1">
                <NumCell value={row.workDays ?? 0} onChange={(v) => setField(idx, 'workDays', v)} />
              </td>
              <td className="px-1">
                <input
                  type="number"
                  ...
                  onChange={(e) => setField(idx, 'coefficient', Number(e.target.value))}
                />
              </td>
              <td className="px-2 text-right text-gray-600">{row.dailyRate != null ? fmt(row.dailyRate) : '—'}</td>
              <td className="px-2 text-right text-gray-800 font-medium">
                {row.computedDailyTotal != null
                  ? fmt(row.computedDailyTotal)
                  : row.totalAmount
                    ? fmt(row.totalAmount)
                    : '—'}
              </td>
      ...
      <p className="text-xs text-gray-400 mt-2">
        Đơn giá ngày = hệ số × lương cơ bản ngày; tổng tiền = số ngày công × đơn giá — hệ thống tự tính khi lưu.
      </p>
    </Card>
  )
}
...
function Empty({ text }: { text: string }) {
  return <EmptyState className="p-8 text-sm">{text}</EmptyState>
}
