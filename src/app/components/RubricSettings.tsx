...
export default function RubricSettings() {
  ...
  const openEdit = (c: RubricCriterionDto) => {
    setEditing(c)
    setForm({
      roundType: ROUND_VALUE[c.roundType],
  ...
  const save = async () => {
    if (!form.name.trim()) {
      setError('Nhập tên tiêu chí')
      return
    }
    if (form.maxScore <= 0) {
      setError('Điểm tối đa phải > 0')
      return
    }
    setSaving(true)
    setError('')
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500 mt-1">
          Cấu hình bộ tiêu chí RIÊNG cho từng loại vòng. Người chấm sẽ thấy đúng các tiêu chí này.
        </p>
      </div>
      ...
      {loading ? (
        ...
        ROUND_TYPES.map((rt) => {
          const items = criteria
            .filter((c) => ROUND_VALUE[c.roundType] === rt.value)
              ...
              <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    {items.length} tiêu chí · tổng {total} điểm
                  </p>
                </div>
                <Button size="sm" onClick={() => openAdd(rt.value)}>
                  ...
                  {items.map((c) => (
                    <div key={c.id} className="px-6 py-3 flex items-center justify-between hover:bg-gray-50">
                      <div className="flex items-center gap-3">
                        ...
                        {!c.isActive && (
                          <span className="text-xs px-2 py-0.5 bg-gray-100 text-gray-500 rounded">ẩn</span>
                        )}
                      </div>
                      <div className="flex items-center gap-3">
